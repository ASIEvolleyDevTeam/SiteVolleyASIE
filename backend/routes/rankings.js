  ...
  try {
    const [rows] = await db.query(
      ...
      SELECT 
        ...
        SUM(CASE WHEN (g.winnerRef IS NOT NULL 
              AND g.winnerRef != t.id 
              AND (g.team1Ref = t.id OR g.team2Ref = t.id)
              AND (g.set1_team1 > 0 OR g.set1_team2 > 0 OR g.set2_team1 > 0 OR g.set2_team2 > 0)) THEN 1 ELSE 0 END) AS defeat,
